...
// On configure Express pour traiter le XML en texte brut
...
app.get("/download/:process_id", (req, res) => {
    ...
    if (jobs[process_id] && jobs[process_id].status === "COMPLETED") {
        res.download(jobs[process_id].csvFile, `output_${process_id}.csv`, (err) => {
            if (err) {
                res.status(500).json({ status: "ERROR", message: "Download error" });
            } else {
                console.log(`CSV downloaded for job ${process_id}`);
            }
        });
    } else {
